...
// Trims every string value (recursively, including nested objects/arrays) so
// accidental leading/trailing spaces from typing or pasting never get saved.
// This matters most for fields used in URLs (SKU, slug) — a trailing space
// there silently breaks the product/project page, since browsers trim
// whitespace from URLs before requesting them, so the request no longer
...
function deepTrim(value) {
  ...
  if (value && typeof value === 'object') {
    const out = {};
    for (const [key, v] of Object.entries(value)) out[key] = deepTrim(v);
    return out;
  }
  return value;
}
...
export function addCategory(category) {
  category = deepTrim(category);
  const categories = getCategories();
  const base = category.slug ? slugify(category.slug) : slugify(category.name?.tr);
  const slug = uniqueSlug(base, categories.map((c) => c.slug));
  const newCategory = { name: { tr: '', en: '', ku: '' }, desc: { tr: '', en: '', ku: '' }, ...category, slug };
...
export function updateCategory(slug, updates) {
  updates = deepTrim(updates);
  const categories = getCategories();
  const idx = categories.findIndex((c) => c.slug === slug);
  if (idx === -1) return null;
  // slug is left as-is on update — products reference categories by slug, so
  // changing it here would silently orphan every product in that category.
  categories[idx] = { ...categories[idx], ...updates, slug: categories[idx].slug };
  writeJSON(CATEGORIES_FILE, categories);
...
// Next.js dynamic route params are supposed to already be URL-decoded, but in
// practice this isn't reliable for segments with multiple separate %XX
// sequences (e.g. a space plus a Turkish "İ") — it can arrive still literally
// percent-encoded, which then fails to match the stored value and 404s even
// though the product/project genuinely exists. Decoding it ourselves, and
// falling back to the raw value if it's not validly encoded, closes that gap.
